...
from app.models import Feedback, Group, Interest, Recommendation, User, UserInterest
...
def test_profile_retrieval_and_update(profile_client) -> None:
    ...
    assert response.json()["user"]["name"] == "Maya Shah"
    ...
    assert updated.json()["user"]["name"] == "Maya Updated"
    assert updated.json()["goals"] == ["create"]
    assert {item["name"] for item in updated.json()["interests"]} == {"Photography", "Writing"}
...
def test_feedback_records_state_and_prevents_duplicates(profile_client) -> None:
    ...
    assert str(group.id) in {item["id"] for item in profile.json()["saved_groups"]}
