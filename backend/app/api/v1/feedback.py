...
def create_feedback(request: FeedbackRequest, session: Session = Depends(get_db)) -> ApiResponse[FeedbackResult]:
    try:
    ...
    return ApiResponse(data=FeedbackResult(feedback_type=request.feedback_type))