...
@router.post("", response_model=IcebreakerResult, summary="Generate a personalized conversation icebreaker")
async def create_icebreaker(request: IcebreakerRequest, _: None = Depends(limit_icebreakers), service: IcebreakerService = Depends(get_icebreaker_service)) -> IcebreakerResult:
    try:
    ...
    return IcebreakerResult(icebreaker=icebreaker, style=request.style)
