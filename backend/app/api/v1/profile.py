...
def get_profile(user_id: UUID, service: ProfileService = Depends(get_profile_service)) -> ApiResponse[ProfileResponse]:
    try:
        return ApiResponse(data=service.get(user_id))
...
def update_profile(user_id: UUID, request: ProfileUpdate, service: ProfileService = Depends(get_profile_service)) -> ApiResponse[ProfileResponse]:
    try:
