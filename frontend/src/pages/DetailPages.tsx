...
import { ArrowLeft, Bookmark, CalendarDays, Check, Heart, MapPin, RefreshCw, Users } from 'lucide-react'
...
export function GroupDetailPage() {
  ...
  function toggleInterested() {
    if (!id) return
    const next = groupService.toggleInterested(id)
    setInterested(next)
    notify(next ? 'You are on the interested list.' : 'Removed from your interested communities.')
  }

  function toggleSaved() {
    if (!id) return
    const next = groupService.toggleSaved(id)
    setSaved(next)
    notify(next ? 'Community saved for later.' : 'Community removed from saved.')
  }

  if (groupQuery.isPending || recommendationsQuery.isPending) return <PageContainer><LoadingState label="Loading community..." /></PageContainer>
  if (groupQuery.isError || recommendationsQuery.isError) return <PageContainer><ErrorState onRetry={() => { void groupQuery.refetch(); void recommendationsQuery.refetch() }} /></PageContainer>
